...
class ModStaff(commands.Cog, name="ModStaff"):
    ...
    async def promote(
        ...
        *,
        ...
        # Roles that are part of the rank ladder are always promotable — they are
        # implicitly staff roles even if not explicitly added via setstaffrole.
        staff_role_ids = cfg.get("staff_role_ids", [])
        rank_order = cfg.get("rank_order", [])
        role_id_str = str(role.id)
        if staff_role_ids and role_id_str not in staff_role_ids and role_id_str not in rank_order:
            return await ctx.send(
                embed=error_embed(
                    "Not a Staff Role",
